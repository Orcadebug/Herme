...
# 加载项目根目录的 .env 文件
# 路径: Hermes/.env (相对于 backend/app/config.py)
...
class Config:
    ...
    def validate(cls):
        ...
        if not cls.SECRET_KEY or cls.SECRET_KEY in {'hermes-secret-key', 'change-me', 'change_me'}:
            errors.append('SECRET_KEY 未配置或仍为默认值')

        if cls.DEBUG:
            errors.append('FLASK_DEBUG 必须为 False 才能以生产模式运行')
