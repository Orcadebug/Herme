...
logger = get_logger("hermes.simulation_runner")

# Flag to indicate whether the cleanup function has been registered
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        if enable_graph_memory_update:
            if not graph_id:
                raise ValueError(
                    "graph_id must be provided when graph memory update is enabled"
                )
        ...
        try:
            ...
            logger.info(
                f": {simulation_id}, pid={process.pid}, platform={platform}"
            )
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    if platform == "twitter":
                                        ...
                                        state.twitter_running = False
                                        logger.info(
                                            f"Twitter : {state.simulation_id}, total_rounds={action_data.get('total_rounds')}, total_actions={action_data.get('total_actions')}"
                                        )
                                    elif platform == "reddit":
                                        state.reddit_completed = True
                                        state.reddit_running = False
                                        logger.info(
                                            f"Reddit : {state.simulation_id}, total_rounds={action_data.get('total_rounds')}, total_actions={action_data.get('total_actions')}"
                                        )
    ...
    def _terminate_process(
        ...
        if IS_WINDOWS:
            ...
            # /F = , /T =
            logger.info(
                f" (Windows): simulation={simulation_id}, pid={process.pid}"
            )
            try:
                #
                subprocess.run(
                ...
                if process.poll() is None:  #
                    ...
                    try:
                        ...
                        if os.path.exists(state_file):
                            ...
                            logger.info(
                                f" state.json  stopped: {simulation_id}"
                            )
                        else:
                            logger.warning(f"state.json : {state_file}")
                    except Exception as state_err:
                        logger.warning(
                            f" state.json : {simulation_id}, error={state_err}"
                        )
    ...
    def interview_agent(
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(
                f"Interview: {simulation_id}"
            )

        logger.info(
            f"Interview: simulation_id={simulation_id}, agent_id={agent_id}, platform={platform}"
        )
    ...
    def interview_agents_batch(
        ...
        logger.info(
            f"Interview: simulation_id={simulation_id}, count={len(interviews)}, platform={platform}"
        )
    ...
    def interview_all_agents(
        ...
        logger.info(
            f"Interview: simulation_id={simulation_id}, agent_count={len(interviews)}, platform={platform}"
        )
