...
logger = get_logger("hermes.oasis_profile")
...
class OasisProfileGenerator:
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.debug(f"Zep {max_retries} : {e}")
            return None
        ...
        try:
            ...
            logger.info(
                f"Zep: {entity_name},  {len(results['facts'])} , {len(results['node_summaries'])} "
            )
...
- """
    ...
    def _print_generated_profile(
        ...
        #
        topics_str = (
            ", ".join(profile.interested_topics) if profile.interested_topics else ""
        )
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        logger.info(
            f" {len(profiles)} Twitter Profile {file_path} (OASIS CSV)"
        )
    ...
    def _save_reddit_json(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        logger.info(
            f" {len(profiles)} Reddit Profile {file_path} (JSONuser_id)"
        )
