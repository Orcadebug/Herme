...
class SportsSimulationState:
    ...
    def to_dict(self) -> Dict[str, Any]:
        data = asdict(self)
        data["status"] = self.status.value if isinstance(self.status, SportsSimulationStatus) else self.status
        return data
...
class SportsSimulationService:
    ...
    def start_simulation_async(self, workspace_id: str) -> str:
        ...
        if workspace.contract_version < SPORTS_WORKSPACE_CONTRACT_VERSION:
            raise ValueError("Workspace was generated by an older fallback-based planner. Re-plan the matchup before simulating.")
        if not self.llm_client:
            ...
            raise RuntimeError(f"Simulation requires a configured LLM client: {self.llm_error or 'unknown error'}")
        if workspace.status not in (SportsWorkspaceStatus.READY, SportsWorkspaceStatus.SIMULATED, SportsWorkspaceStatus.REPORT_READY):
        ...
        thread = threading.Thread(target=self._run_worker, args=(workspace_id, simulation_id), daemon=True)
        thread.start()
        return simulation_id

    def get_state(self, workspace_id: str, simulation_id: Optional[str] = None) -> Optional[SportsSimulationState]:
        workspace = SportsWorkspaceManager.get_workspace(workspace_id)
        if not workspace:
            return None
    ...
    def get_events(self, workspace_id: str, simulation_id: Optional[str] = None) -> List[Dict[str, Any]]:
        workspace = SportsWorkspaceManager.get_workspace(workspace_id)
        if not workspace:
            return []
        simulation_id = simulation_id or workspace.latest_simulation_id
    ...
    def _create_state(self, simulation_id: str, workspace: SportsWorkspace) -> SportsSimulationState:
        ...
        home_players = [item["name"] for item in players if item["team"] == workspace.home_team["name"]]
        away_players = [item["name"] for item in players if item["team"] == workspace.away_team["name"]]
        home_coaches = [item["name"] for item in coaches if item["team"] == workspace.home_team["name"]]
        away_coaches = [item["name"] for item in coaches if item["team"] == workspace.away_team["name"]]
    ...
    def _run_worker(self, workspace_id: str, simulation_id: str) -> None:
        ...
        try:
            ...
            while step < state.target_steps:
                ...
                if not self._is_basketball_state(state):
                    state.current_segment = self._segment_for_step(workspace.rule_pack, step)
                    event = self._generate_event(workspace, state, events[-3:])
                    self._apply_event(state, event, workspace.rule_pack)
                else:
                    event_payload = self.basketball_engine.step(workspace, state, events[-6:], rng)
    ...
    def _generate_event(
        ...
        raise RuntimeError(f"Simulation event generation failed after retries: {last_error}")
    ...
    def _is_basketball_state(self, state: SportsSimulationState) -> bool:
        return state.simulation_mode in {"basketball_multi_agent", "basketball_llm_multi_agent"}
    ...
    def _generate_event_with_llm(
        ...
        home_excerpt = self._lineup_excerpt(workspace, state.home_lineup[:3], state.home_coach)
        away_excerpt = self._lineup_excerpt(workspace, state.away_lineup[:3], state.away_coach)
        recent_summary = "\n".join(f"- {event.play_by_play}" for event in recent_events) or "- No recent events"
    ...
    def _event_from_payload(
        ...
        if segment != state.current_segment:
            raise ValueError(f"Event segment '{segment}' does not match current segment '{state.current_segment}'")
        ...
        offense_team_name = state.home_team if offense_team == "home" else state.away_team
        offense_players = set(state.home_lineup if offense_team == "home" else state.away_lineup)
        offense_coaches = {
            item["name"] for item in workspace.participants
        ...
        if primary_actor not in offense_players:
            raise ValueError(f"Primary actor '{primary_actor}' is not a player on the offense team")

        secondary_actor = str(payload.get("secondary_actor", "") or "").strip()
        if secondary_actor and secondary_actor not in all_participants:
            raise ValueError(f"Secondary actor '{secondary_actor}' is not a known participant")

        coach = self._required_str(payload, "coach")
        if coach not in offense_coaches:
            raise ValueError(f"Coach '{coach}' is not a known coach for the offense team")
    ...
    def _apply_event(self, state: SportsSimulationState, event: SportsEvent, rule_pack: Dict[str, Any]) -> None:
        allowed_values = sorted(int(value) for value in rule_pack.get("allowed_score_values", [0, 1, 2, 3]))
        max_delta = int(rule_pack.get("max_delta_per_step", max(allowed_values)))
        if event.segment != state.current_segment:
            raise ValueError(f"Event segment '{event.segment}' does not match current segment '{state.current_segment}'")
    ...
    def _lineup_excerpt(self, workspace: SportsWorkspace, lineup: List[str], coach_name: str) -> str:
        excerpts = [f"Coach: {coach_name}"]
        for player_name in lineup:
            participant = next((item for item in workspace.participants if item["name"] == player_name), None)
            line = f"- {player_name}"
    ...
    def _normalize_side_label(self, value: str, state: SportsSimulationState, field_name: str) -> str:
    ...
    def _state_path(self, workspace_id: str, simulation_id: str) -> str:
        return os.path.join(SportsWorkspaceManager.get_simulations_dir(workspace_id), simulation_id, "state.json")

    def _events_path(self, workspace_id: str, simulation_id: str) -> str:
        return os.path.join(SportsWorkspaceManager.get_simulations_dir(workspace_id), simulation_id, "events.json")

    def _save_state(self, state: SportsSimulationState) -> None:
        os.makedirs(os.path.dirname(self._state_path(state.workspace_id, state.simulation_id)), exist_ok=True)
        with open(self._state_path(state.workspace_id, state.simulation_id), "w", encoding="utf-8") as f:
    ...
    def _save_events(self, simulation_id: str, events: List[SportsEvent]) -> None:
        if not events:
            ...
            path = self._events_path(self._state_cache[simulation_id].workspace_id, simulation_id)
