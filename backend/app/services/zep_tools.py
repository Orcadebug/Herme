...
Encapsulates graph search, node reading, edge query and other tools for Report Agent

Core search tools (optimized):
1. InsightForge (Deep Insight Retrieval) - Most powerful hybrid search, auto-generates sub-questions and multi-dimensional retrieval
...
logger = get_logger("hermes.zep_tools")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Breadth Search (Future Panorama View)",
            f"Query: {self.query}",
            f"\n### Statistics",
        ...
        if self.active_facts:
            text_parts.append(
                f"\n### [Current Active Facts] (Original simulation results)"
            )
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out garbage content with question numbers (Question 1-9)
                skip = False
                for d in "123456789":
...
class ZepToolsService:
    ...
    [Core Search Tools - Optimized]
    1. insight_forge - Deep Insight Retrieval (most powerful, auto-generates sub-questions, multi-dimensional)
    2. panorama_search - Breadth Search (full picture, including expired content)
    3. quick_search - Simple Search (fast retrieval)
    4. interview_agents - In-Depth Interview (interview simulated agents for multi-perspective views)
    ...
    def search_graph(
        ...
        Uses hybrid search (semantic + BM25) to find related information in the graph.
    ...
    def get_all_nodes(self, graph_id: str) -> List[NodeInfo]:
        """
        Get all graph nodes (paginated fetch)
    ...
    def get_all_edges(
        ...
        """
        Get all graph edges (paginated fetch, with temporal info)
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all graph edges, then filters those related to the specified node
    ...
    def insight_forge(
        ...
        Most powerful hybrid search function, automatically decomposes questions and performs multi-dimensional retrieval:
        ...
        5. Consolidate all results to generate deep insights
        ...
        # Get details of all related entitiesLimit quantity
        entity_insights = []
        ...
        # Step 4: Build all relationship chainsLimit quantity
        relationship_chains = []
    ...
    def _generate_sub_queries(
        ...
        Decompose complex problems into multiple independently retrievable sub-questions
        """
        system_prompt = """You are a professional problem analysis expert. Your task is to decompose a complex problem into multiple sub-questions that can be independently observed in the simulation world.

Requirements:
1. Each sub-question should be specific enough to find related Agent behavior or events in the simulation world
    ...
    def interview_agents(
        ...
        try:
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                #
                twitter_text = (
                    twitter_response if twitter_response else ""
                )
                reddit_text = (
                    reddit_response if reddit_response else ""
                )
...
3-5"""
        ...
        try:
            ...
            return response.get(
                "questions", [f"{interview_requirement}"]
            )
