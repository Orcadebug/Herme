...
- After simulation completes, does not close environment immediately; enters command-waiting mode
...
# This fixes the issue where OASIS third-party library reads files without specifying encoding
# ============================================================
import sys
import os
...
if sys.platform == "win32":
    ...
    # Force default encoding (affects open() function's default encoding)
...
# Load .env file from project root (includes LLM_API_KEY and other configs)
...
# Add the filter immediately when module loads, ensuring it takes effect before camel code executes
logging.getLogger().addFilter(MaxTokensWarningFilter())


def disable_oasis_logging():
    """
    Disable OASIS library verbose log output
...
class ParallelIPCHandler:
    ...
    async def handle_interview(
        ...
        if platform in ("twitter", "reddit"):
            ...
            if "error" in result:
                ...
                print(
                    f"  Interview: agent_id={agent_id}, platform={platform}, error={result['error']}"
                )
                return False
            else:
        ...
        if success_count > 0:
            ...
            print(
                f"  Interview: agent_id={agent_id}, ={success_count}/{len(platforms_to_interview)}"
            )
            return True
        else:
            errors = [
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            ...
            print("")
            self.send_response(
                command_id, "completed", result={"message": ""}
            )
            return False

        else:
            self.send_response(
                command_id, "failed", error=f": {command_type}"
            )
            return True
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    print(
        f"{config_label} model={llm_model}, base_url={llm_base_url[:40] if llm_base_url else ''}..."
    )
...
async def run_twitter_simulation(
    ...
    if max_rounds is not None and max_rounds > 0:
        ...
        if total_rounds < original_rounds:
            log_info(
                f": {original_rounds} -> {total_rounds} (max_rounds={max_rounds})"
            )
...
async def main():
    ...
    parser.add_argument("--twitter-only", action="store_true", help="Twitter")
    parser.add_argument("--reddit-only", action="store_true", help="Reddit")
    parser.add_argument(
        "--max-rounds",
        type=int,
        default=None,
